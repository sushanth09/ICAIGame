...
/**
 * GlobalSoundToggle — the ONE sound button for the entire app.
 * Lives in GameLayout so it renders on every page.
 * Manages:
 *   - Ambient music (Cmaj9 oscillator pad + pentatonic arpeggio)
 *   - Sound-effects enabled flag (localStorage, default ON)
 *   - Cross-page sync via custom "soundToggle" DOM event
 */
...
const PAD_NOTES = [
  { freq: 65.41,  gain: 0.10 },
  { freq: 98.00,  gain: 0.08 },
...
export function GlobalSoundToggle() {
  ...
  const toggle = useCallback(() => {
    ...
    setIsOn(nextOn);
    setSoundEnabled(nextOn); // persists to localStorage + fires "soundToggle" event
  }, [isOn, initAudio, scheduleArp]);

  /* ── Sync if another component fires the event ───────────────────── */
  useEffect(() => {
